...
class HeroesHandlerTestCase(unittest.TestCase):
    ...
    def test_get_heroes(self):
        ...
        for i in range(1, 21):
            self.create_hero(f"Hero {i}", "marvel")
    ...
    def test_get_hero(self):
        ...
        # Enviando a req para obter o heroi
        response = self.app.get(f"/hero/{hero_id}")
        self.assertEqual(response.status_code, 200)

        # Pegando o json de resposta
    ...
    def test_hero_not_found(self):
        ...
        # Envinado a req para obter o heroi
        response = self.app.get("/hero/id_aleatorio")

        # A req vai voltar 404 pois nao existe nenhum heroi com esse id
        self.assertEqual(response.status_code, 404)

        # Json retornado
        self.assertDictEqual(response.get_json(), {"message": "Hero not found"})
    ...
    def test_update_hero(self):
        ...
        response = self.app.post(path=f"/hero/{hero.id}", json=params)
    ...
    def test_create_hero_without_name(self):
        """Test create a hero withou a name"""
        params = {
            "hero": {
                "name": "",
                "description": "",
    ...
    def test_create_hero_with_name_formatted(self):
        """Test create a hero with uppercase name and blank spaces"""
        params = {
            "hero": {
                "name": "SUPERMAN",
                "description": "Hero description",
        ...
        # Obtendo o heroi no bda para conferir o nome
    ...
    def test_create_hero_with_invalid_universe(self):
        ...
        self.assertEqual(response.status_code, 500)
        self.assertEqual(
            response.get_json()["details"], "Bad request, invalid universe"
        )

    def test_create_hero_with_valid_url(self):
        """Test create hero with valid url"""
        params = {
            "hero": {
                "name": "Catwoman",
                "description": "Hero description",
                "universe": "dc",
                "imageUrl": "htp/??link?img?jpg",
            }
        }
        response = self.app.post(path="/heroes", json=params)
        self.assertEqual(response.status_code, 200)

        valid_url = Hero.get_hero(response.get_json()["id"])
        self.assertTrue(valid_url.imageUrl)

    def test_create_hero_with_description_formatted(self):
        """Test create a hero with uppercase name and black spaces"""
        params = {
            "hero": {
                "name": "BATMAN",
                "description": "HERO DESCRIPTION",
        ...
        # Obtendo o heroi no bda para conferir a descrição
        description_updated = Hero.get_hero(response.get_json()["id"])
        self.assertEqual(description_updated.description, "Hero Description")
